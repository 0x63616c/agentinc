...
The Ticket label gap changes from 6 to 8 logical px and its button horizontal inset from 10 to 12 px, matching Automation. Both keep the existing `CONTROL_HEIGHT` value of 32. The shared field input inset remains 12 px. The header search's 9/4 px, sidebar identity's 6.5/6 px, and Evee header's 10/9 px optical insets are named in `ui/tokens.rs`; the concurrent sidebar lane retains ownership of its call site.
