//! Secrets live in the macOS login Keychain. They never reach plain files, logs,
//! product records, workflow history or API responses.
use anyhow::Result;
...
/// Generic passwords in the login Keychain under `<bundle ID>.<area>`, so the
/// development and production channels never share credentials.
pub struct Keychain {
    service: String,
}
impl Keychain {
    pub fn for_area(area: &str) -> Self {
        Self {
            service: format!("{}.{area}", ainc_release::identity::BUNDLE_ID),
        }
    }
}
#[cfg(target_os = "macos")]
...
mod tests {
    ...
    fn memory_store_round_trips_and_forgets() {
        ...
        store.set("token", "fixture").unwrap();
        assert_eq!(store.get("token").unwrap().as_deref(), Some("fixture"));
