...
A Conversation reply and inline inference need no Ticket. Autonomous work with effects does: Evee or a person creates and assigns a Ticket through the same authorized command. A Ticket has one assignee and six statuses: **Backlog**, **To do**, **In progress**, **Blocked**, **Done**, **Cancelled**, plus a priority, labels, its order on the board and relationships to other Tickets. Assignment of actionable (To do or In progress) work dispatches the agent; starting work moves it to In progress, and completion records a Comment and moves it to Done. Failure or waiting for input is run state, not another Ticket status. Revision checks and assignment generations prevent a stale run from mutating a reassigned Ticket. Comments are the work log; streaming tokens remain run events.
